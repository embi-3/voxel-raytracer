...
Scene voxelise(const std::string& filePath) {
    ...
    for (size_t i = 0; i < data.model.voxels.size(); i++) {
        ...
        Colour currVoxelColour =
            Colour(data.palette[currColourIndex].r, data.palette[currColourIndex].g, data.palette[currColourIndex].b, data.palette[currColourIndex].a);

        Voxel currVoxel = Voxel(currVoxelColour);
        Coordinate currVoxelCoord =
            Coordinate(data.model.voxels[i].x + 1, data.model.voxels[i].z + 1, data.model.voxels[i].y + 1
