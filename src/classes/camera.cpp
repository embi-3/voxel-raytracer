...
namespace renderer {
    std::vector<Pixel> Camera::render(const Scene& scene, const Shader& shader) const {
        auto progress = 0;
    ...
    std::vector<Pixel> Camera::render(const VoxelGrid& grid, const Shader& shader) const {
        auto pixels = std::vector<Pixel>{}; // array of pixels
