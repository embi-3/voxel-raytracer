...
namespace geometry {
    ...
    void SVOVoxelGrid::set_voxel(Coordinate coords, Voxel voxel) {
        ...
        for (std::size_t depth = 0; depth < max_depth; depth++) {
            ...
            if (!child) {
                ...
                auto min = Vec3(curr->bounding_box.min.x + static_cast<double>(x_bit) * (x_dist / 2),
    ...
    IntersectionList SVOVoxelGrid::traverse(const Ray& ray) const {
        ...
        while (!stack.empty()) {
            ...
            std::sort(sorted_children.begin(), sorted_children.end(), [](const StackNode& a, const StackNode& b) {
