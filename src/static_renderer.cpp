...
void createPPM(std::size_t width, std::size_t height, const std::vector<Pixel>& pixels) {
    ...
    for (const auto& pixel : pixels) {      
        auto r_int = static_cast<int>(255.999 * pixel.r);
        auto g_int = static_cast<int>(255.999 * pixel.g);
        auto b_int = static_cast<int>(255.999 * pixel.b);
        file << r_int << " " << g_int << " " << b_int << " ";
    }
}

// simple gradient from 4.2 from "Ray Tracing In One Weekend" website
texture::Colour ray_colour(const geometry::Ray& r) {
    geometry::Vec3 unit_direction = r.direction().into_unit();
    auto a = 0.5 * (unit_direction.y + 1.0);
    return (1.0 - a) * texture::Colour(1.0, 1.0, 1.0) + a * texture::Colour(0.5, 0.7, 1.0);
}
...
int main() {
    ...
    // render
    auto pixels = std::vector<texture::Colour>{};   // array of pixels
    for (int j = 0; j < image_height; j++) {
        for (int i = 0; i < image_width; i++) {
            ...
            texture::Colour pixel_colour = ray_colour(r);
            pixels.push_back(pixel_colour);
        }
    }
    createPPM(static_cast<std::size_t>(image_width), static_cast<std::size_t>(image_height), pixels);
}
